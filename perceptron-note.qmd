---
title: "Perceptroner, Adaline og Sigmoid"
...
vores krydset ved et rødt parti.
...
fig <- ggplot(dat6, aes(x=x, y=y)) + 
  ...
  theme_minimal() +
  geom_segment(aes(x=x1, y=y1, xend=x2, yend=y2), data = data.frame(x1=c(-2.2,0), x2 = c(2.2,0), y1 = c(0, -1.2), y2 = c(0, 2.14)), arrow = arrow(length = unit(0.2,"cm"), type = "closed")) +
...
Svar på en skala fra -2 til 2 på følgende spørgsmål: $$\begin{aligned}
x_1 &: \text{Jeg synes, at indkomstskatten skal sættes ned} \\
x_2 &: \text{Jeg synes ikke, at danske virksomheder skal pålægges en CO2-afgift}\end{aligned}$$
...
sættes ned. Det vil sige, at $x_1=0$. Samtidig er denne vælger enig i,
at danske virksomheder ikke skal pålægges en CO2-afgift. Altså er
$x_2=1$. Vi udregner nu:
$$
-1.5+1.2 \cdot x_1+x_2=-1.5+1.2 \cdot 0+1=-0.5
$$
Og da denne værdi er mindre end $0$, sætter vi $o=-1$. Det vil sige, at vi vil anbefale denne vælger at stemme rød blok.*
:::
...
\begin{aligned}
    ...
    w_1 \leftarrow w_1 + & error \cdot x_1 \\
    w_2 \leftarrow w_2 + & error \cdot x_2 \\
    & \vdots  \\
    w_n \leftarrow w_n + & error \cdot x_n \\
\end{aligned}
$$
...
$$
\begin{aligned}
w_i \leftarrow w_i + error \cdot x_i,
\end{aligned}
$$
...
$$
\begin{aligned}
w_0 &\leftarrow 0.5 -2 = -1.5 \\
w_1 &\leftarrow 0.5 -2 \cdot (-1) = 2.5 \\
w_2 &\leftarrow 0.5 -2 \cdot 1= -1.5 \\
\end{aligned}
$$
...
$$
\begin{aligned}
w_0+w_1\cdot x_1 + w_2 \cdot w_2 = -1.5 +2.5 \cdot (-1) -1.5 \cdot (-1) = -2.5.
\end{aligned}
$$
...
vægtene ikke har ændret sig, så stopper man! I vores lille eksempel sker
det faktisk ret hurtigt, og man ender med følgende vægte:
$$
\begin{aligned}
w_0 = 0.5, w_1 = 4.5, w_2=0.5
\end{aligned}
$$
Det svarer til ligningen
$$
\begin{aligned}
0.5 +4.5 \cdot x_1 + 0.5 \cdot x_2 = 0.
\end{aligned}
$$
...
#| message: false
#| warning: false
#| fig-cap: Illustration af svaret på spørgsmål 1 ($1.$ aksen) og spørgsmål 2 ($2.$ aksen) med en markering af om man vil stemme på rød eller blok blå. Her er indtegnet to linjer, som kan separere de blå punkter fra de røde. Den sorte linje er vores eget skøn, mens den grønne stammer fra perception learning algoritmen (med startværdier $w_0=0.5$, $w_1=0.5$ og $w_2=0.5$).
#| label: fig-punkter_kandidattest_graf3
fig + geom_abline(slope=-1.2,intercept=1.5,size=1) + geom_abline(slope=-9,intercept=-1,colour='green',size=1)
```
...
@exm-Rosenblatts. Vi har stadig, at $x_1=0$ og $x_2=1$. Vi udregner nu med de nye vægte:
$$
0.5+4.5 \cdot x_1+0.5 \cdot x_2=0.5+4.5 \cdot 0+0.5 \cdot 1=1
$$
Nu har vi en værdi, som er større end $0$, og derfor sætter vi $o=1$. Det vil
sige, at vores anbefaling til denne vælger nu vil være at stemme på blå
...
#| message: false
#| warning: false
#| fig-cap: Illustration af svaret på spørgsmål 1 ($1.$ aksen) og spørgsmål 2 ($2.$ aksen) med en markering af om man vil stemme på rød eller blok blå. Her er indtegnet to linjer, som kan separere de blå punkter fra de røde. Den sorte linje er vores eget skøn, mens den grønne stammer fra perception learning algoritmen (med startværdier $w_0=0.5$, $w_1=0.5$ og $w_2=0.5$).Punktet med koordinatsæt $(0,1)$ er markeret med sort.
#| label: fig-punkter_kandidattest_graf4
fig + geom_abline(slope=-1.2,intercept=1.5,size=1) + geom_abline(slope=-9,intercept=-1,colour='green',size=1)+geom_point(aes(x=0,y=1),colour="black",size=3)
```

*Det sorte punkt i $(0,1)$ repræsenterer vælgerens svar på de to
spørgsmål. Her kan vi se, at punktet ligger under den sorte linje
svarende til, at vi med denne klassificering anbefaler vælgeren at
stemme på rød blok. Samtidig ligger punktet over den grønne linje og
...
$$
\begin{aligned}
w_i \leftarrow w_i + error \cdot x_i = w_i + 2 \cdot x_i.
\end{aligned}
$$
...
$$
\begin{aligned}
y_{new} = (w_0+2) + (w_1+2 \cdot x_1) \cdot x_1 + \cdots + (w_n+2 \cdot x_n) \cdot x_n,
\end{aligned}
$$
...
$$
\begin{aligned}
y_{new} &= w_0+2 + w_1\cdot x_1 + 2\cdot x_1^2 + \cdots + w_n \cdot x_n + 2\cdot x_n^2 \\
&=w_0 + w_1 \cdot x_1 + \cdots + w_n \cdot x_n + 2 + 2\cdot x_1^2 + \cdots 2\cdot x_n^2 \\
&=y_{old} + 2(1 + x_1^2 + \cdots +x_n^2).
\end{aligned}
$$
Men da
$$
\begin{aligned}
2(1 + x_1^2 + \cdots +x_n^2) >0,
\end{aligned}
$$
så må
$$
...
$$
hvor
vi egentlig gerne ville have haft, at denne værdi havde været negativ. I
...
\end{aligned}
$$
Her er $\eta$ (udtales "eta") en lille værdi (f.eks. $0.05$), som
...
hvor langsomme, vi bevæger os ned mod minimum. Af den grund kaldes
$\eta$ også for en *learning rate* - fordi den afgører, hvor hurtigt vi
